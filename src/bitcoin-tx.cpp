...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s goldcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  goldcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded goldcoin transaction") + "\n" +
              "  goldcoin-tx [options] -create [commands]   " + _("Create hex-encoded goldcoin transaction") + "\n" +
              "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, getSTANDARD_SCRIPT_VERIFY_FLAGS(), MutableTransactionSignatureChecker(&mergedTx, i)))
            fComplete = false;
    }
