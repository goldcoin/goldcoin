...
#include <thread>     // C++11 std::thread replaces boost::thread

// Include the Signal class from validationinterface.h
...
public:
    ...
    Signal<void(TorControlConnection &,const TorControlReply &)> async_handler;
private:
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!conn.Connect(_target, [this](TorControlConnection& conn) { connected_cb(conn); },
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                // _conn.Command("AUTHENTICATE " + HexStr(status_cookie.second), [this](auto& conn, auto& reply) { auth_cb(conn, reply); });
                cookie = std::vector<uint8_t>(status_cookie.second.begin(), status_cookie.second.end());
                clientNonce = std::vector<uint8_t>(TOR_NONCE_SIZE, 0);
                GetRandBytes(&clientNonce[0], TOR_NONCE_SIZE);
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce),
                             [this](TorControlConnection& conn, const TorControlReply& reply) {
                                 authchallenge_cb(conn, reply);
                             });
            } else {
...
void TorController::Reconnect()
{
    ...
    if (!conn.Connect(target,
                     [this](TorControlConnection& conn) { connected_cb(conn); },
                     [this](TorControlConnection& conn) { disconnected_cb(conn); })) {
