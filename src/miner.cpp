...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    LogPrintf("CreateNewBlock(): total size: %u txs: %u fees: %ld sigops %d\n", nSerializeSize, nBlockTx, nFees, nBlockSigOpsCost);
    ...
    for (const CTxMemPool::txiter it : alreadyAdded) {
        ...
        for (CTxMemPool::txiter desc : descendants) {
            ...
            if (mit == mapModifiedTx.end()) {
                ...
                CTxMemPoolModifiedEntry modEntry = *mit;
                mapModifiedTx.erase(mit);
                // Update for parent inclusion
                modEntry.nSizeWithAncestors -= it->GetTxSize();
                modEntry.nModFeesWithAncestors -= it->GetModifiedFee();
                modEntry.nSigOpCostWithAncestors -= it->GetSigOpCost();
                mapModifiedTx.insert(modEntry);
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated)
{
    ...
    auto mi = mempool.mapAncestorScore.begin();
    ...
    while (mi != mempool.mapAncestorScore.end() || !mapModifiedTx.empty())
    {
        // First try to find a new transaction in mapTx to evaluate.
        if (mi != mempool.mapAncestorScore.end() &&
                SkipMapTxEntry(mi->second, mapModifiedTx, failedTx)) {
            ++mi;
            continue;
        }
        ...
        modtxscoreiter modit = mapModifiedTx.begin();
        if (mi == mempool.mapAncestorScore.end()) {
...
void BlockAssembler::addPriorityTxs()
{
    ...
    {
        double dPriority = mi->second.GetPriority(nHeight);
        CAmount dummy;
        mempool.ApplyDeltas(mi->second.GetTx().GetHash(), dPriority, dummy);
        vecPriority.push_back(TxCoinAgePriority(dPriority, CTxMemPool::txiter_wrapper(mi, mempool.mapTx)));
    }
