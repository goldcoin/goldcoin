...
bool OldEncrypt(const CKeyingMaterial& vchPlaintext, std::vector<unsigned char> &vchCiphertext, const unsigned char chKey[32], const unsigned char chIV[16])
{
    ...
    // EVP_CIPHER_CTX_init is not needed with EVP_CIPHER_CTX_new (already initialized)
    if (fOk) fOk = EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, chKey, chIV) != 0;
    if (fOk) fOk = EVP_EncryptUpdate(ctx, &vchCiphertext[0], &nCLen, &vchPlaintext[0], nLen) != 0;
    if (fOk) fOk = EVP_EncryptFinal_ex(ctx, (&vchCiphertext[0]) + nCLen, &nFLen) != 0;
    // EVP_CIPHER_CTX_cleanup is replaced by EVP_CIPHER_CTX_reset in OpenSSL 1.1.0+
    // But since we're freeing the context right after, we can skip it
...
#else
    // OpenSSL 1.1.0+ uses OpenSSL_version_num() instead of SSLeay()
    if (vchDecrypted1 != vchDecrypted2 && vchDecrypted1.size() >= AES_BLOCK_SIZE && OpenSSL_version_num() == 0x100010afL)
#endif
    {
