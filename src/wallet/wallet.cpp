...
bool CWallet::Verify()
{
    ...
    uiInterface.TriggerInitMessage(_("Verifying wallet..."));
    ...
    if (!bitdb.Open(GetDataDir()))
    {
        ...
        try {
            fsbridge::Rename(pathDatabase, pathDatabaseBak);
    ...
    if (fsbridge::Exists(GetDataDir() / walletFile))
    {
...
CBlockIndex* CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        TriggerWalletShowProgress(_("Rescanning..."), 0); // show rescan progress in GUI as dialog or on splashscreen, if -rescan on startup
        double dProgressStart = GuessVerificationProgress(chainParams.TxData(), pindex);
        double dProgressTip = GuessVerificationProgress(chainParams.TxData(), chainActive.Tip());
        while (pindex)
        {
            if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0)
                TriggerWalletShowProgress(_("Rescanning..."), std::max(1, std::min(99, (int)((GuessVerificationProgress(chainParams.TxData(), pindex) - dProgressStart) / (dProgressTip - dProgressStart) * 100))));
        ...
        TriggerWalletShowProgress(_("Rescanning..."), 100); // hide progress dialog in GUI
    }
    return ret;
}
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (coinControl && !std::holds_alternative<CNoDestination>(coinControl->destChange))
...
void CWallet::GetScriptForMining(std::shared_ptr<CReserveScript> &script)  // C++23 shared_ptr
{
    std::shared_ptr<CReserveKey> rKey = std::make_shared<CReserveKey>(this);  // C++23 make_shared
    CPubKey pubkey;
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (GetBoolArg("-zapwallettxes", false)) {
        uiInterface.TriggerInitMessage(_("Zapping all transactions from wallet..."));
    ...
    uiInterface.TriggerInitMessage(_("Loading wallet..."));
    ...
    if (chainActive.Tip() && chainActive.Tip() != pindexRescan)
    {
        ...
        uiInterface.TriggerInitMessage(_("Rescanning..."));
    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(strWalletFile) || bitdb.mapFileUseCount[strWalletFile] == 0)
            {
                ...
                if (fsbridge::IsDirectory(pathDest))
                    pathDest /= strWalletFile;

                try {
                    // Use std::filesystem copy with overwrite option
                    std::error_code ec;
                    fs::copy_file(pathSrc, pathDest, fs::copy_options::overwrite_existing, ec);
                    if (ec) {
                        throw fs::filesystem_error("copy failed", pathSrc, pathDest, ec);
                    }
