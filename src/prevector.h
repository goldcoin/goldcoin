...
public:
    ...
    explicit prevector(size_type n) : _size(0), _union{} {
        resize(n);
    }

    explicit prevector(size_type n, const T& val = T()) : _size(0), _union{} {
        change_capacity(n);
        while (size() < n) {
            _size++;
            new(static_cast<void*>(item_ptr(size() - 1))) T(val);
    ...
    template<typename InputIterator>
    prevector(InputIterator first, InputIterator last) : _size(0), _union{} {
        size_type n = last - first;
        change_capacity(n);
        while (first != last) {
            _size++;
    ...
    prevector(const prevector<N, T, Size, Diff>& other) : _size(0), _union{} {
