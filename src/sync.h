...
#include <mutex>              // C++23 std::mutex, std::recursive_mutex
#include <condition_variable> // C++23 std::condition_variable
#include <shared_mutex>       // C++23 std::shared_mutex
...
CCriticalSection mutex;
    std::recursive_mutex mutex;  // C++23
...
/**
 * C++23 recursive mutex: supports recursive locking, but no waiting
...
/** C++23 mutex: supports waiting but not recursive locking */
using CWaitableCriticalSection = AnnotatedMixin<std::mutex>;

/** C++23 condition variable */
...
/** C++23 std::unique_lock<Mutex> wrapper */
