    ...
    consoleFontSize(0)
{
    ...
    connect(ui->clearButton, &QPushButton::clicked, this, &RPCConsole::clear);    connect(ui->fontBiggerButton, &QPushButton::clicked, this, &RPCConsole::fontBigger);    connect(ui->fontSmallerButton, &QPushButton::clicked, this, &RPCConsole::fontSmaller);    connect(ui->btnClearTrafficGraph, &QPushButton::clicked, ui->trafficGraph, &TrafficGraphWidget::clear);
...
void RPCConsole::setClientModel(ClientModel *model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        // Replace QSignalMapper with lambdas for Qt 6 compatibility
        ...
        connect(ui->peerWidget->selectionModel(), &QItemSelectionModel::selectionChanged,
            this, &RPCConsole::peerSelected);
        // peer table signal handling - update peer details when new nodes are added to the model
        connect(model->getPeerTableModel(), &QAbstractItemModel::layoutChanged, this, &RPCConsole::peerLayoutChanged);        // peer table signal handling - cache selected node ids
        connect(model->getPeerTableModel(), &QAbstractItemModel::layoutAboutToBeChanged, this, &RPCConsole::peerLayoutAboutToChange);        
        // set up ban table
        ...
        connect(ui->banlistWidget, &QWidget::customContextMenuRequested, this, &RPCConsole::showBanTableContextMenu);
        connect(unbanAction, &QAction::triggered, this, &RPCConsole::unbanSelectedNode);
        // ban table signal handling - clear peer details when clicking a peer in the ban table
        connect(ui->banlistWidget, &QTableView::clicked, this, &RPCConsole::clearSelectedNode);        // ban table signal handling - ensure ban table is shown or hidden (if empty)
        connect(model->getBanTableModel(), &QAbstractItemModel::layoutChanged, this, &RPCConsole::showOrHideBanTableIfRequired);        showOrHideBanTableIfRequired();
...
void RPCConsole::startExecutor()
{
    ...
    connect(this, &RPCConsole::stopExecutor, &thread, &QThread::quit);    // - queue executor for deletion (in execution thread)
...
void RPCConsole::banSelectedNode(int bantime)
{
	...
	if(stats) {
	    g_connman->Ban(stats->nodeStats.addr, BanReason::ManuallyAdded, bantime);
	}
    }
    clearSelectedNode();
