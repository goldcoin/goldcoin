...
#include <memory>  // For std::unique_ptr instead of boost::scoped_array
#include <fstream>
...
// UTF-8 handling is automatic in std::filesystem
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("goldcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("goldcoin://", Qt::CaseInsensitive))
    {
...
{
    QString ret = QString("goldcoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Goldcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Goldcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Goldcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoin*.lnk
    return fsbridge::Exists(StartupShortcutPath());
}

bool SetStartOnSystemStartup(bool fAutoStart)
{
    // If the shortcut exists already, remove it for updating
    fsbridge::Remove(StartupShortcutPath());
...
#ifdef UNICODE
            std::unique_ptr<TCHAR[]> args(new TCHAR[strArgs.length() + 1]);
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "goldcoin.desktop";
    return GetAutostartDir() / strprintf("goldcoin-%s.lnk", chain);
}
...
{
    if (!fAutoStart)
        fsbridge::Remove(GetAutostartFilePath());
    else
    {
        ...
        fsbridge::CreateDirectories(GetAutostartDir());
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Goldcoin\n";
        else
            optionFile << strprintf("Name=Goldcoin (%s)\n", chain);
...
// C++23 std::filesystem handles UTF-8 correctly on all platforms
