...
const char* BIP71_MIMETYPE_PAYMENT = "application/goldcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/goldcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/goldcoin-paymentrequest";
#endif
...
#endif
#else // ENABLE_BIP70
    // BIP70 is disabled, so we don't need to load root CAs
    (void)_store; // Suppress unused parameter warning
#endif // ENABLE_BIP70
}
...
#else
        Q_EMIT message(tr("Payment request file handling"),
            tr("Payment protocol support is disabled in this build."),
            CClientUIInterface::ICON_WARNING);
#endif
        return;
    }
...
#else
    // BIP70 is disabled, just emit error
