...
static constexpr uint64_t BLOCK_CHAIN_SIZE = 6;
/* Minimum free space (in GB) needed for data directory when pruned; Does not include prune target */
static constexpr uint64_t CHAIN_STATE_SIZE = 2;
...
void Intro::startThread()
{
    ...
    connect(this, &Intro::requestCheck, executor, &FreespaceChecker::check);    /*  make sure executor object is deleted in its own thread */
    connect(this, &Intro::stopThread, executor, &QWidget::deleteLater);    connect(this, &Intro::stopThread, thread, &QThread::quit);
    thread->start();
}
