...
*Setup instructions for a Gitian build of Goldcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Goldcoin
...
- [Building Goldcoin Core](#building-goldcoin-core)
...
Clone the git repositories for goldcoin and Gitian.
...
in the goldcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Goldcoin Core
----------------

To build Goldcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the goldcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/goldcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/goldcoin-project/goldcoin
...
For example:
```bash
URL=https://github.com/thrasher-/goldcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit goldcoin=${COMMIT} --url goldcoin=${URL} ../goldcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit goldcoin=${COMMIT} --url goldcoin=${URL} ../goldcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit goldcoin=${COMMIT} --url goldcoin=${URL} ../goldcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the goldcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../goldcoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url goldcoin=${BTCPATH},signature=${SIGPATH} ../goldcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
[goldcoin-project/gitian.sigs.GLD](https://github.com/goldcoin-project/gitian.sigs.GLD/) repository, or if that's not possible create a pull
