...
# Copyright (c) 2023 The Goldcoin Core developers
...
# Dependencies
ossPatchUrl="https://www.goldcoinproject.org/files/depends/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://www.goldcoinproject.org/files/depends/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://www.goldcoinproject.org/files/depends/MacOSX10.11.sdk.tar.gz"
...
Standalone script to perform the gitian build of Goldcoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/goldcoin/goldcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/goldcoin-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/goldcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean goldcoin git directory because of old caching
    if [ -d inputs/goldcoin/ ]; then
        echo "Cleaning Goldcoin directory..."
        rm -rf inputs/goldcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit goldcoin="$COMMIT" --url goldcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
