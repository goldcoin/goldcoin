...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8121
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8122
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18121
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18122
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8121
	%{_sbindir}/semanage port -d -p tcp 8122
	%{_sbindir}/semanage port -d -p tcp 18121
	%{_sbindir}/semanage port -d -p tcp 18122
	for selinuxvariant in %{selinux_variants}; do
